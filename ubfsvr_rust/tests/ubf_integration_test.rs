...
fn test_ubftest() {
    ...
    let message = response.get_string(T_MESSAGE_FLD, 0).expect("Failed to get message");
    assert_eq!(message, "Hello, Rust!");
    
    let status = response.get_string(T_STATUS_FLD, 0).expect("Failed to get status");
    assert_eq!(status, "OK");
}
...
fn test_ubfadd() {
    ...
    let name = response.get_string(T_NAME_FLD, 0).expect("Failed to get name");
    assert_eq!(name, "John Doe");
    
    let id = response.get_long(T_ID_FLD, 0).expect("Failed to get ID");
    assert_eq!(id, 12345);
    
    let count = response.get_long(T_COUNT_FLD, 0).expect("Failed to get count");
    assert_eq!(count, 100);
    
    let price = response.get_double(T_PRICE_FLD, 0).expect("Failed to get price");
...
fn test_ubfget() {
    ...
    ubf.add_string(T_NAME_FLD, "Test User").expect("Failed to add name");
