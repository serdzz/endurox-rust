...
fn generate_ubf_constants() {
    ...
    if let Ok(entries) = fs::read_dir(&ubftab_dir) {
        for entry in entries.flatten() {
            ...
            if let Some(ext) = path.extension() {
                if ext == "h" && path.file_name().unwrap().to_str().unwrap().ends_with(".fd.h") {
                    found_files = true;
    ...
    if !found_files {
        println!("cargo:warning=No *.fd.h files found in ubftab/, skipping UBF constants generation");
        return;
    }
    ...
    file.write_all(rust_code.as_bytes()).expect("Failed to write ubf_fields.rs");
